...
void play_percent_sudoku()
{
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (first_token != NULL) {
            int var = atoi(first_token);
            char *second_token = strtok(NULL, " \t\n");
            
            // 如果是单子句（第二个token是0或NULL）且变量为正
            if (var > 0 && var <= 729 && second_token != NULL && atoi(second_token) == 0) {
                int i = (var - 1) / 81;
                int j = ((var - 1) % 81) / 9;
                int k = ((var - 1) % 9) + 1;
                puzzle[i][j] = k;
...
int generate_complete_sudoku(int grid[9][9])
{
    ...
    printf("Random generation failed after 100 attempts\n");
    return 0;
}
...
int validate_percent_sudoku_move(int grid[9][9], int row, int col, int num)
{
    ...
    // 检查反对角线约束 (0,8), (1,7), (2,6), (3,5), (4,4), (5,3), (6,2), (7,1), (8,0)
    if (row + col == 8) {
        for (int i = 0; i < 9; i++) {
            if (i != row && grid[i][8-i] == num) return 0;
        }
    }
...
void sudoku_to_cnf(int grid[9][9], const char *filename)
{
    ...
    // 正确计算子句数量
    // 格约束: 81个格子，每个格子1个正子句 + 36个负子句 = 81 * 37 = 2997
    // 行约束: 9行 * 9数字 * (1个正子句 + 36个负子句) = 9 * 9 * 37 = 2997  
    // 列约束: 9列 * 9数字 * (1个正子句 + 36个负子句) = 9 * 9 * 37 = 2997
    // 盒子约束: 9盒子 * 9数字 * (1个正子句 + 36个负子句) = 9 * 9 * 37 = 2997
    // 反对角线约束: 9数字 * (1个正子句 + 36个负子句) = 9 * 37 = 333
    // 窗口1约束: 9数字 * (1个正子句 + 36个负子句) = 9 * 37 = 333
    // 窗口2约束: 9数字 * (1个正子句 + 36个负子句) = 9 * 37 = 333
    // 提示数字: hints个单子句
    int clause_count = 2997 + 2997 + 2997 + 2997 + 333 + 333 + 333 + hints;
