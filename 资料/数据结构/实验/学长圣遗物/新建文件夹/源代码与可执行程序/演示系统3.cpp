...
/*--------------此为分割线---------------*/
void creat(BiTree &T, TElemType definition[], int *count) {
    if (definition[*count].key != 0) {
        T = (BiTree)malloc(sizeof(BiTNode));
        T->data = definition[*count];
        (*count)++;
        creat(T->lchild, definition, count);
        creat(T->rchild, definition, count);
    } else {
        T = NULL;
        (*count)++;
    }
...
status CreateBiTree(BiTree &T, TElemType definition[]) {
    ...
    for (int i = 0; i < n; i++) {
        if (definition[i].key != 0) {
            int current_key = definition[i].key;
            int found = 0;
 ...
 break;
            ...
            keys[count_keys++] = current_key;
        }
    }

    int count = 0;
    creat(T, definition, &count);
